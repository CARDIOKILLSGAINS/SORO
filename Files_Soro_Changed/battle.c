...
static struct Damage battle_calc_multi_attack(struct Damage wd, struct block_list *src,struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if( sd && !skill_id ) {	// if no skill_id passed, check for double attack [helvetica]
		short i;
		if( ( ( skill_lv = pc_checkskill(sd,TF_DOUBLE) ) > 0 && sd->weapontype1 == W_1HSWORD )
...
static int battle_calc_attack_skill_ratio(struct Damage wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch( skill_id )
	{
		...
		case RA_WUGDASH:// ATK 300%
			...
			break;
		case RA_WUGSTRIKE:
			//skillratio = 200 * skill_lv; //Original [Defect]
			skillratio = 35 * skill_lv;
			break;
		case RA_WUGBITE:
	...
	switch (skill_id) {
		...
		case RA_WUGBITE:
			if(sd)
				...
				atk = (5*pc_checkskill(sd, RA_TOOTHOFWUG));
...
#else
		//divided by 2 now - DEFECT VIT DEFENSE SOFT DEFENSE SORO CUSTOM
		vit_def = def2;
#endif
...
struct Damage battle_calc_attack_left_right_hands(struct Damage wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	if (sd) {
		...
		else if(sd->status.weapon == W_2HSWORD && !skill_id) { //2hSwords offhand damage only applies to normal attacks //I THINK THIS IS CUSTOM THAT WE SHOULD REMOVE [Defect] [SORO]
			skill = pc_checkskill(sd,SM_TWOHAND);
			wd.damage2 = (int64)wd.damage * (1 + (skill * 2))/100;
		}
		else if(is_attack_right_handed(src, skill_id) && is_attack_left_handed(src, skill_id)) {	//Dual-wield
//REMOVED Class requirements, removed KO_Right calculation, increased base from 50 to 60. [Defect] [SORO]
			if (wd.damage) {
					skill = pc_checkskill(sd,AS_RIGHT);
					ATK_RATER(wd.damage, 60 + (skill * 10))
				if(wd.damage < 1) wd.damage = 1;
			}
			if (wd.damage2) {
					skill = pc_checkskill(sd,AS_LEFT);
					ATK_RATEL(wd.damage2, 40 + (skill * 10)) //Increase base to 40
...
//SORO CUSTOM SETTINGS
	{ "stat_vit_defb",                  &battle_config.stat_vit_defb,                   100,    0,      INT_MAX,        },
	{ "pc_max_status_def",                  &battle_config.pc_max_sc_def,                   100,    0,      INT_MAX,        },
};
