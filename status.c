...
void status_calc_misc(struct block_list *bl, struct status_data *status, int level)
{
	...
	//Def2
	stat = status->def2;
	stat += (int)(((float)level + status->vit)/2 + ((float)status->agi/5)); // base level + (every 2 vit = +1 def) + (every 5 agi = +1 def) DEFECT Changed SOFT DEF to 5 for vit, 10 for agi
	...
	status->flee = cap_value(stat,1,SHRT_MAX);
	//Def2 SORO CUSTOM SOFT DEF [Defect]
	stat = status->def2;
	stat += status->vit/battle_config.stat_vit_defb;
	status->def2 = cap_value(stat,0,SHRT_MAX);
	//MDef2
	stat = status->mdef2;
...
// ----- STATS CALCULATION -----
	...
	// PASSIVES THAT GRANT STATS [DEFECT] [SORO]
...
// ----- ASPD CALCULATION -----
	...
	//CUSTOM SORO LINE
	if((skill=pc_checkskill(sd,SBR_SHARPEN))>0)
		status->aspd_rate += 3*skill;
	else if(pc_isridingdragon(sd))
		status->aspd_rate += 250-50*pc_checkskill(sd,RK_DRAGONTRAINING);
#else // Needs more info
	//ASPD PASSIVE [DEFECT] [SORO]
	if((skill=pc_checkskill(sd,SA_ADVANCEDBOOK))>0 && sd->status.weapon == W_BOOK)
		status->aspd_rate += 5*skill;
	if((skill = pc_checkskill(sd,SG_DEVIL)) > 0 && !pc_nextjobexp(sd))
		status->aspd_rate += 30*skill;
